(() => {
  ...
  // --- mount inside the delivery block, never as a floating banner ---
  // OpenCart 3 default checkout is a stepped accordion whose steps load via
  ...
  const hideNativeAddress = () => {
    // Only declutter when we sit inside the Delivery Details address form:
    // hide the native field rows (keep the Continue button + our widget) so the
    // customer sees a single clean Nova Poshta block. Inputs stay in the DOM
    // (display:none) and are auto-filled so the order still validates. When we
    // mount in the Delivery Method step there is nothing to hide.
    const host = wrap.closest('#collapse-shipping-address');
    if (!host) return;
    const form = host.querySelector('form');
    if (!form) return;
    [...form.querySelectorAll('.form-group')].forEach((g) => {
      if (g.contains(wrap)) return;
      g.classList.add('np-native-hidden');
    });
  };
