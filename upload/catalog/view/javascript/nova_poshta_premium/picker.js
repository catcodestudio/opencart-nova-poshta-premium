(() => {
  ...
  const gate = () => {
    ...
    if (isNp && _wasHidden && window.__ccMethodPicked) setTimeout(function () { try { wrap.scrollIntoView({ behavior: 'smooth', block: 'center' }); } catch (e) {} }, 350);
  };
  const wireGate = () => {
  ...
  const seedNative = () => {
    ...
    setTimeout(() => {
      ...
      if (zone && !zone.value) {
        ...
        // Oblast'", and that seed survives into the order whenever the oblast
        // of the chosen city finds no match in the store's zone list.
